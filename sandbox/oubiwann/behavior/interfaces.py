...
class IEmotionalAppraisal:
    ...
    Je = the set of all agents relevant to emotion e. J1 is the set consisting
        only of the self. J2 is the set consisting of everyone but the self. J
        is the union of J1 and J2.
    Cijkl = a list of paths through the ith ontology of agent j triggered to
        condition l (0=success or 1=failure) by stake k.
    Wij(Cijkl) = the weighted importance of of the values of agent j that
        succeed and fail in one's ith concern set.
    f1(rjk) = a function that captures the strength of positive and negative
        relationships one has with the ja agents of objects that are effected
        or spared in state k.
    f2(O,N) = a function that captures the temporal factors of the state and
        how to discount and merge one's emotions from the past, in the present
        and for the future.
    e = emtional pairs; there are 11 pairs of these, oppositely valenced (e.g.
        pride-shame, hope-fear).

    Equation:

    Ie(Sk) = Sum over each j belonging to Je of the Sum over each c belonging
        to Cijkl the product ( Wij(c) * f1(rjk) * f2(O,N) )
